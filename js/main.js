...
function initGamesFilterAndSearch() {
  const gamesContainer = document.querySelector('#games .row.g-4');
  if (!gamesContainer) return;

  // Insert Filter Buttons and Search Bar above games
  const filterSection = document.createElement('div');
  filterSection.className = 'col-12 mb-4';
  filterSection.innerHTML = `
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-center gap-3 bg-white p-3 rounded-4 shadow-sm border">
      <div class="btn-group btn-group-sm flex-wrap gap-1" role="group" id="gameCategoryFilters">
        <button type="button" class="btn btn-outline-dark rounded-pill px-3 active" data-category="all">All Games</button>
        <button type="button" class="btn btn-outline-dark rounded-pill px-3" data-category="sports">Sports</button>
        <button type="button" class="btn btn-outline-dark rounded-pill px-3" data-category="fighting">Fighting</button>
        <button type="button" class="btn btn-outline-dark rounded-pill px-3" data-category="action">Action</button>
        <button type="button" class="btn btn-outline-dark rounded-pill px-3" data-category="adventure">Adventure</button>
      </div>
      <div class="input-group input-group-sm style-search-input" style="max-width: 280px;">
        <span class="input-group-text bg-transparent border-end-0"><i class="bi bi-search text-muted"></i></span>
        <input type="text" id="gameSearchInput" class="form-control border-start-0 ps-0" placeholder="Search game title...">
      </div>
    </div>
  `;
  gamesContainer.parentNode.insertBefore(filterSection, gamesContainer);

  const filterBtns = filterSection.querySelectorAll('#gameCategoryFilters button');
  const searchInput = filterSection.querySelector('#gameSearchInput');
  ...
  function renderGames() {
    ...
    if (filtered.length === 0) {
      gamesContainer.innerHTML = `
        <div class="col-12 text-center py-5">
          ...
          <p class="text-muted mt-2">No games found matching your search.</p>
        </div>
      `;
      return;
    }
    ...
    gamesContainer.innerHTML = filtered.map(game => `
      <div class="col-6 col-md-3">
        <div class="game-card h-100 position-relative" data-game-id="${game.id}">
          <div class="game-img-wrapper">
            <span class="game-overlay-badge">PS5 4K</span>
            <img src="${game.img}" alt="${game.title}" class="game-img">
          </div>
          <div class="p-3 text-center d-flex flex-column justify-content-between">
            <div>
              <h4 class="game-title mb-1">${game.title}</h4>
              <div class="d-flex justify-content-center align-items-center gap-1 mb-2">
                <span class="badge bg-${game.category === 'sports' ? 'danger' : game.category === 'fighting' ? 'warning text-dark' : 'primary'} capitalize">${game.category}</span>
                <span class="badge bg-light text-dark border"><i class="bi bi-star-fill text-warning me-1"></i>${game.rating}</span>
              </div>
            </div>
            <button class="btn btn-sm btn-ps-outline w-100 mt-2 btn-game-details" data-id="${game.id}">
    ...
    gamesContainer.querySelectorAll('.btn-game-details').forEach(btn => {
...
/* ----------------------------------------------------
 * 4. Interactive Live Rate & Bill Estimator Calculator
 * ---------------------------------------------------- */
